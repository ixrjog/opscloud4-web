...
export default {
  ...
  methods: {
    ...
    init () {
      ...
      term.open(document.getElementById(this.container.instanceId))
      this.fitAddon.fit() // 获取对象的高度和宽度
      term.focus() // 聚焦
      if (this.container.sessionType === 'CONTAINER_TERMINAL') {
        const _this = this
        term.onData(function (cmd) {
          const command = {
            command: cmd,
            state: _this.terminalState.COMMAND,
            instanceId: _this.container.instanceId
          }
    ...
    resize () {
      ...
      // this.$nextTick(() => {
      // })
      const id = this.container.instanceId
      this.fitAddon.fit() // 获取对象的高度和宽度
      const resizeMessage = {
        ...
        terminal: {
          ...
          height: document.getElementById(this.container.instanceId).clientHeight
        }
      }

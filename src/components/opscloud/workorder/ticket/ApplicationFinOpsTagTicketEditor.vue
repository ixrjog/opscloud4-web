        ...
        <el-timeline-item timestamp="工单选项" placement="top">
          <el-card shadow="hover">
            <ticket-fin-ops-entry-selector v-if="ticketView.ticketPhase === 'NEW'"
                                           ...
                                           :finOpsTagOptions="finOpsTagOptions"
                                           @handleNotify="fetchData"/>
            <ticket-fin-ops-entry-table :ticketId="ticketView.ticketId"
...
export default {
  ...
  methods: {
    initData (ticketView) {
      ...
      this.getFinOpsTagOptions()
      this.$nextTick(() => {
        this.fetchData()
      }, 200)
    },
    getFinOpsTagOptions () {
      GET_FINOPS_TAG_OPTIONS()
        .then(res => {
          this.finOpsTagOptions = res.body.options
        })
    },
    /**
     * 提交工单

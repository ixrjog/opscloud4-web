        ...
        <el-timeline-item timestamp="工单选项" placement="top">
          <el-card shadow="hover">
            <ticket-kubernetes-deployment-entry-selector v-if="ticketView.ticketPhase === 'NEW'"
                                                         ...
                                                         @handleNotify="fetchData"></ticket-kubernetes-deployment-entry-selector>
            <ticket-entry-table :ticketId="ticketView.ticketId"
              ...
              <template v-slot:extend>
                <el-table-column prop="entry" label="当前副本数">
                  <template slot-scope="scope">
                    <span>{{ scope.row.entry.replicas }}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="entry" label="缩容后副本数">
                  <template slot-scope="scope">
                    <el-input-number size="mini"
                                     :disabled="ticketView.ticketPhase !== 'NEW'"
                                     :min="1"
                                     :max="scope.row.entry.replicas -1"
                                     v-model="scope.row.entry.reduceReplicas"
                                     @change="updateEntry(scope.row)"></el-input-number>
                  </template>
                </el-table-column>
              </template>
            </ticket-entry-table>
...
export default {
  ...
  name: 'ApplicationReduceReplicasTicketEditor',
  props: ['formStatus'],
  components: {
    TicketTitle,
    NodeView,
  ...
  methods: {
    ...
    updateEntry (row) {
      row.properties = {
        reduceReplicas: row.entry.reduceReplicas
      }

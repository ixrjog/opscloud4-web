      ...
      <el-timeline>
        <el-timeline-item timestamp="WorkOrder Options" placement="top">
          <el-card shadow="hover">
            <ticket-aws-transfer-create-user-form :workOrderTicketId="ticketView.ticketId" ref="ticketAwsTransferCreateUserForm"
            ...
            <ticket-entry-table :ticketId="ticketView.ticketId"
              ...
              <template v-slot:extend>
                <el-table-column label="SshPublicKey">
                  <template v-slot="scope">
                    <span>{{ scope.row.entry.sshPublicKey }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="Transfer Server">
                  <template v-slot="scope">
                    <span>
                      {{ scope.row.entry.userName }}@s-6d384561f0da4b148.server.transfer.eu-west-1.amazonaws.com
                    </span>
                  </template>
                </el-table-column>
              </template>
...
import TicketAwsTransferCreateUserForm from '@/components/opscloud/workorder/child/TicketAwsTransferCreateUserForm.vue'

const TableLayout = {
  instance: false,
  entryName: 'Transfer UserName'
}
...
export default {
  ...
  name: 'AwsTransferCreateUserTicketEditor',
  props: ['formStatus'],
  components: {
    TicketTitleI18n,
    TicketEntryTable,
    NodeView,
    TicketAwsTransferCreateUserForm,
    WorkflowNodes
  },
  mixins: [],
  mounted () {
  ...
  methods: {
    initData (ticketView) {
      ...
      this.$nextTick(() => {
        if (JSON.stringify(ticketView.ticketEntries) === '[]') {
          this.$refs.ticketAwsTransferCreateUserForm.initDate()
        } else {
          this.$refs.ticketEntryTable.initData(ticketView)
        }
      }, 200)
    },
    /**

      ...
      <el-timeline>
        <el-timeline-item :timestamp="$t('workOrder.workOrderOptions')" placement="top">
          <el-card shadow="hover">
            <ticket-ons-topic-v5-form :workOrderTicketId="ticketView.ticketId" ref="ticketOnsTopicV5Form"
...
import TicketOnsTopicV5Form from '@/components/opscloud/workorder/child/TicketOnsTopicV5Form'
...
export default {
  ...
  name: 'OnsTopicV5TicketEditor',
  props: ['formStatus'],
  components: {
    TicketTitleI18n,
    TicketOnsTopicV5Form,
    NodeView,
    TicketEntryTable,
    WorkflowNodes
  },
  ...
  methods: {
    initData (ticketView) {
      ...
      this.$nextTick(() => {
        if (this.ticketView.ticketPhase === 'NEW') {
          this.$refs.ticketOnsTopicV5Form.getDsInstance()
        }
        this.fetchData()
      }, 200)
    },
    ...
    ticketEntriesChanged (ticketEntries) {
      this.$refs.ticketOnsTopicV5Form.initDate(ticketEntries)
    }
  }
}
</script>
